...
class DataIngestion:
  ...
  def download_data(self):
    ...
    try: 
      ...
      zip_file_name = os.path.basename(dataset_url) #extracts the file name from the url: books.zip
      zip_file_path = os.path.join(zip_download_dir, zip_file_name)
      logging.info(f"Downloading data from {dataset_url} into file {zip_file_path}")
      urllib.request.urlretrieve(dataset_url, zip_file_path) #downloads the file and save them to the zip_file_path
